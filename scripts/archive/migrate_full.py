...
Comprehensive 14-phase migration script from `e2-demo-field-eng` to
`fe-sandbox-felix-demo-sandbox`. Superseded by `scripts/deploy_agents_fevm.py`
(targets `fevm-felix-demo`, uses the current Agent Bricks REST paths:
`/api/2.1/knowledge-assistants`, `/api/2.1/supervisor-agents`, `/api/2.0/data-rooms/`).

Do not run against a live workspace. The paths here (`/api/2.0/agent-bricks/*`)
have been removed on newer workspaces. See `docs/lessons-learned.md` §11 for
the updated paths.

Usage (historical):
    python scripts/archive/migrate_full.py <phase|all>
    python scripts/archive/migrate_full.py 0        # Pre-flight checks
    python scripts/archive/migrate_full.py all      # All 14 phases
"""
