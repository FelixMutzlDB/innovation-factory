...
23. **MAS agent-naming cleanup (added 2026-04-23).** Recreate AdTech and HB MAS with consistent `snake_case` sub-agent `name` fields, a consistent `"{Domain} Supervisor"` display-name suffix, and instructions that reference sub-agents by their machine name rather than a human label. Probing showed the current MAS emits mixed tool name styles (`genie_data_explorer`, `transfer_to_issue_resolution_specialist`, `query_agent`, `call_agent_tool`, `product_identifier_agent`, `query_agent_quality_auth_analyst`) because my Phase-7 POST used mixed sub-agent names. Fix: rewrite `scripts/deploy_agents_fevm.py::phase_7` with snake-case names + delete-and-recreate idempotency (PATCH doesn't reliably update the `agents` array). Invocation test for each routed sub-agent per MAS. ~45min.
24. **Migration story: hybrid DAB + Python bootstrap (added 2026-04-23).** So we can recreate this project in a fresh workspace in ~20 min. Keep `deploy_agents_fevm.py` as the imperative gap-filler for anything DAB doesn't cover natively (Agent Bricks KA/MAS, Genie spaces, workspace settings, dashboard migration). Move the Lakebase database, UC catalog/schemas/volumes, dashboards (via `resources.dashboards`), and env vars into `databricks.yml` as first-class DAB resources with multiple targets (`dev`, `prod`, `demo-<workspace>`). Add `scripts/bootstrap.py` + `Makefile` that runs `bundle deploy` → `deploy_agents_fevm.py all` → emits env vars to paste back into `databricks.yml`. Terraform considered and rejected: agent-bricks support lags, and we'd duplicate DAB responsibility for the app. ~4–6h.
