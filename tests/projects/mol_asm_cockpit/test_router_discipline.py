"""Router-discipline regression test for mol_asm_cockpit.

Mirrors ``tests/common/test_router_discipline.py`` but scoped to the
mol_asm_cockpit project tree.

Contract: every ``@router.(get|post|patch|delete|put)`` decorator under
``src/innovation_factory/backend/projects/mol_asm_cockpit/`` MUST declare both
(1) ``operation_id=`` and (2) one of ``response_model`` / ``response_class`` /
``responses``, OR be additionally decorated with ``@streaming_endpoint``.

Per CLAUDE.md: "All API routes need response_model and operation_id —
required for TypeScript client generation."
"""
...
# Debt grandfathered at project creation. Add entries only with a TODO
# referencing a backlog ticket; never let this grow indefinitely.
...
def _collect_violations() -> list[tuple[Path, int, str, str, set[str]]]:
    ...
    for py_path in MAC_DIR.rglob("*.py"):
...
def test_mol_asm_routes_have_operation_id_and_response_declaration():
    """Every mol_asm_cockpit route must declare operation_id + a response shape."""
    ...
    if cleared:
        ...
        pytest.fail(
            ...
            "tests/projects/mol_asm_cockpit/test_router_discipline.py:\n"
            + "\n".join(cleared_lines)
        )
    if new_violations:
        lines = [
        ...
        pytest.fail(
            f"\nFound {len(new_violations)} NEW mol_asm_cockpit route(s) violating "
            "router-discipline (CLAUDE.md). Either fix or add to KNOWN_DEBT:\n"
            + "\n".join(lines)
        )


def test_mol_asm_known_debt_is_empty():
    """mol_asm_cockpit was delivered without router debt. The allowlist
    stays empty by policy. Add entries only with a backlog ticket."""
    assert KNOWN_DEBT == set(), (
        f"mol_asm_cockpit KNOWN_DEBT must stay empty. Found: {KNOWN_DEBT}"
    )
